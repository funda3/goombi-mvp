...
export function NearbyServices({ listing, onShowOnMap }: Props) {
  ...
  useEffect(() => {
    ...
    fetchNearbyServices(listing.latitude, listing.longitude)
      .then((data) => { cacheRef.current.set(listing.id, data); setGroups(data); })
  ...
  return (
    <div className="mt-4 border-t border-slate-200 pt-4">
      ...
      {expanded && (
        <div className="mt-3">
          {loading && <p className="text-xs text-slate-500">Fetching from OpenStreetMap…</p>}
          ...
          {!loading && !error && groups.length > 0 && !hasResults && (
